...
# Create Packages and Packages.gz for each architecture
for ARCH in "${ARCHITECTURES[@]}"; do
  ...
  echo "Created Packages and Packages.gz for $ARCH"
done
...
if find "$SOURCE_DIR" -name "*.dsc" | grep -q .; then
  ...
  echo "Created Sources and Sources.gz"
fi
...
{
  echo "MD5Sum:"
  find dists/stable -type f ! -name "Release" ! -name "Release.gpg" ! -name "InRelease" -exec md5sum {} \; | sed -e 's@dists/stable/@@' | awk '{ print " " $1 " " length($2) " " $2 }'
  
  echo "SHA256:"
  find dists/stable -type f ! -name "Release" ! -name "Release.gpg" ! -name "InRelease" -exec sha256sum {} \; | sed -e 's@dists/stable/@@' | awk '{ print " " $1 " " length($2) " " $2 }'
} >> "$RELEASE_FILE"
